...
impl ReceiveAndBuffer for TransactionViewReceiveAndBuffer {
    ...
    fn receive_and_buffer_both(
        ...
        let timeout = if container.is_empty() {
            ...
            if elapsed >= self.batch_interval {
                ...
                let remaining = self.batch_interval - elapsed;
                ...
                match self.receiver.recv_timeout(timeout) {
                    ...
                    Err(RecvTimeoutError::Timeout) => {
                        match self.verified_bundle_receiver.try_recv() {
                            ...
                            Err(TryRecvError::Disconnected) => {
                                return (!self.batch.is_empty() || !self.bundle_batch.is_empty())
                                    .then_some(stats)
                                    .ok_or(DisconnectedError);
                            }
                        }
        ...
        let drain_timeout = Duration::from_micros(500);
